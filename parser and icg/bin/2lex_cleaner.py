...
#        print([x[1] for x in l])
        if tok.value not in [x[1] for x in l] :
            ...
            ln = tok.lineno - 1
            temp_l = [ x[0] for x in al if int(x[2])==ln ]
            while len(temp_l)>0 and temp_l[0] == 'WHITE' and str(tok.value) not in temp_l and 'DEF' not in temp_l :
                ln -= 1
                temp_l = [ x[0] for x in al if int(x[2])==ln ]
                
            temp_l = [x[0] for x in al if int(x[2]==ln)]     

            if  len(temp_l)>0 and temp_l[0] == 'DEF' and str(tok.value) not in temp_l:            
                tok.type = 'LOCAL_ID'
                line = str(tok.type)+'\t'+str(tok.value)+'\t'+str(tok.lineno)+'\t'+str(tok.lexpos)+'\n'
                st.write(line)
                l.append([tok.type,tok.value,tok.lineno,tok.lexpos])
